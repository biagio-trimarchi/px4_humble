...
Eigen::MatrixXd GaussianProcessMatern32::getK() {
	return K;
}
...
Eigen::MatrixXd GaussianProcessMatern32::getDataY() {
	return data_y;
}
...
void GaussianProcessMatern32::loadData(const Eigen::MatrixXd& inputK,
	...
	if ((inputK.cols() != input_data_x.cols()) || (input_data_x.cols() != input_data_y.rows())) {
		std::cerr << "GPMatern32 : Matrix dimensions do not coincide. Loading failed" << std::endl;
		return;
	}
