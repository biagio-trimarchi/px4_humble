...
void DoubleIntegratorGovernor::stateMachine() {
	switch(agent_state) {
		case INIT:
			...
			if (message_received) {
				RCLCPP_INFO(this->get_logger(), "[INIT] Start received");
				message_received = false;
				setModeOffboard();
				arm();
			}
		...
		case HOVER:
			...
			RCLCPP_INFO(this->get_logger(), "[HOVER]: Error: %lf", std::abs(drone_orientation.angularDistance(q_reference_yaw)) );
			if (message_received) {
				message_received = false;
				agent_state = ROTATE_RIGHT;
				reference_position = Eigen::Vector3d(0.0, 0.0, takeoff_altitude);
				reference_yaw = -M_PI/2.0;
				q_reference_yaw = Eigen::Quaterniond(std::cos(reference_yaw/2.0), 0.0, 0.0, std::sin(reference_yaw/2.0));
			}
			break;

		case ROTATE_RIGHT:
			RCLCPP_INFO(this->get_logger(), "[ROTATE RIGHT]: Rotating");
			RCLCPP_INFO(this->get_logger(), "[ROTATE RIGHT]: Error: %lf", std::abs(drone_orientation.angularDistance(q_reference_yaw)) );
			RCLCPP_INFO(this->get_logger(), "[ROTATE RIGHT]: Quaternion refence: %lf %lf %lf %lf", q_reference_yaw.w(), q_reference_yaw.x(), q_reference_yaw.y(), q_reference_yaw.z() );
			RCLCPP_INFO(this->get_logger(), "[ROTATE RIGHT]: Quaternion actual: %lf %lf %lf %lf", drone_orientation.w(), drone_orientation.x(), drone_orientation.y(), drone_orientation.z() );
			if (transition_setpoint_reached) {
