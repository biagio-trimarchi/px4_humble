...
void CameraToOccupancy::pointcloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& msg_pointcloud, const nav_msgs::msg::Odometry::ConstSharedPtr& msg_odometry) {
	...
	box_filter.setFilterLimits(pointcloud_filter_min_distance, pointcloud_filter_max_distance / 2.0);
	box_filter.setFilterFieldName("x");
	box_filter.filter(current_pointcloud);
	...
	// Align the empty space pointcloud with the map
	Eigen::Quaterniond yaw_orientation = yawRotationQuaternion(drone_orientation);
	Eigen::Vector3d drone_plane_position(drone_position.x(), drone_position.y(), 0.0);
	Eigen::Affine3d odom2freespace_pointcloud = Eigen::Affine3d::Identity();
