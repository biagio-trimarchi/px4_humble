#include <double_integrator_test/double_integrator_moving_obs.hpp>
...
DoubleIntegratorGovernor::DoubleIntegratorGovernor() : Node("Governor") {
	...
	moving_obstacle_publisher = this->create_publisher<geometry_msgs::msg::Twist>(
				"/model/MovingSphere1/cmd_vel", 10
			);
...
void DoubleIntegratorGovernor::moveObstacle() {
	double v_x_0 = -0.2;
	double v_z_0 = 2 * std::sqrt(9.8);

	time_trajectory_obstacle += double(dynamics_timer_frequency_ms) * 0.001;
	double v_x = v_x_0;
	double v_z = v_z_0 * time_trajectory_obstacle - 4.9 * time_trajectory_obstacle * time_trajectory_obstacle;
	...
	moving_obstacle_publisher->publish(msg);
	RCLCPP_INFO(this->get_logger(), "Moving object...");
}
